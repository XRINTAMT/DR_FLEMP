...
namespace Meta.WitAi
{
    ...
    public class WitRequest
    {
        ...
        {
            Debug.Log("Someone just made this request: " + action.Method.Name);
        ...
        private System.Collections.IEnumerator PerformMainThreadCallbacks()
        {
            ...
            while (HasMainThreadCallbacks())
            {
                ...
                {
                    if ((result.Method.Name != "<HandleResponse>b__2") || final)
                    {
                        Debug.Log("IS it final? " + final);
                        Debug.Log(result.Method.Name + " compared to " + "<HandleResponse>b__2");
                        result();
                    }
                    else
                    {
                        Debug.Log("It was trying to end it all, but we would not let it");
                    }
                }
